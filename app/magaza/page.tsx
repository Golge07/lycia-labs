  ...
  return (
    <main className="relative z-10 space-y-8 bg-[var(--background)] px-6 pb-16 pt-10 text-foreground md:px-10 lg:px-14">
      <section className="space-y-3">
        ...
        <p className="text-[rgba(59,43,43,0.75)]">Kategori filtresi ve ID bazlı detay yönlendirmesi hazır.</p>
        <div className="flex flex-wrap gap-3 pt-2">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setActive(cat.id)}
              className={`rounded-full px-4 py-2 text-sm transition shadow-sm ${
                active === cat.id
          ...
          >
            ...
            <div className="space-y-2 px-5 py-4">
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <span className="text-xs uppercase tracking-[0.2em] text-[rgba(59,43,43,0.6)] group-hover:text-terracotta">
                  Detay
                </span>
              </div>
            </div>
