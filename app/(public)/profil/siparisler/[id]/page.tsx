      ...
      {order ? (
        <div className="mt-5 space-y-4">
          ...
          <div className="space-y-3 sm:hidden">
            {order.items.map((i) => (
              <div key={i.id} className="rounded-2xl border border-foreground/10 bg-white/80 p-4">
                <div className="flex items-start gap-3">
                  <div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-foreground/10 bg-white/70">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {i.product_id ? <p className="mt-1 text-xs text-foreground/60">ID: {i.product_id}</p> : null}
                    <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-foreground/80">
                      <div className="rounded-xl bg-foreground/5 px-3 py-2">
                        <p className="text-xs uppercase tracking-[0.2em] text-foreground/60">Adet</p>
                        <p className="mt-1 font-semibold text-foreground">{i.quantity}</p>
                      </div>
                      <div className="rounded-xl bg-foreground/5 px-3 py-2">
                        <p className="text-xs uppercase tracking-[0.2em] text-foreground/60">Birim</p>
                        <p className="mt-1 font-semibold text-foreground">{formatMoney(i.unit_price)}</p>
                      </div>
                      <div className="col-span-2 rounded-xl bg-terracotta/10 px-3 py-2">
                        <p className="text-xs uppercase tracking-[0.2em] text-foreground/60">Tutar</p>
                        <p className="mt-1 font-semibold text-terracotta">{formatMoney(i.line_total)}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="hidden overflow-x-auto rounded-2xl border border-foreground/10 bg-white/80 sm:block">
